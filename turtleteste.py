...
def eqReta (x1,y1, x2,y2, esp, cor): 
    ...
    if (m > 1): 
        if (y1 > y2): 
            ...
            if (x1 == x2): 
                ponto(x1, y1, esp, "yellow") 
                y = y2 
                for y in range(y2,y1): 
                    ponto(x1, y, esp, "black") 
            else: 
                ponto(x2, y2, esp, "purple") 
                y = y2 
                for y in range (y2, y1): 
                    ponto(((y-b)/m), y, esp, "black") 
        elif (x1 == x2): 
            print("Intervalo em Y eh maior com reta vertical") 
            ponto(x1, y1, esp, "pink") 
            y = y1 
            for y in range (y1,y2): 
                ponto(x1, y, esp, "pink") 
        else: 
            print("Intervalo em Y eh maior com x1 < x2") 
            ponto (x1, y1, esp, "cyan") 
            y = y1 
            for y in range (y1, y2): 
                ponto(((y-b)/m), y, esp, "black") 
    else: 
        if(x1>x2): 
            ...
            ponto(x2, y2, esp, "black") 
            x = x2 
            for x in range(x2, x1): 
                ponto(x, (b+(m*x)), esp, "red") 
        elif(x1 == x2): 
